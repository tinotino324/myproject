...
<div class="box-inner">
    <div class="box-header">
        ...
        <div class="box-header d-flex align-items-center justify-content-start">                
            ...
            <h3 class="box-title mb-0">Books</h3>         
        </div>   
            <div class="d-flex align-items-center gap-3 site-card-bg px-4 padding-site-cards justify-content-center rounded-2 question-card-width card-width">
                <div>
                    ...
                    <p class="primary-text-dark mb-0 text-center">Total Books</p>
                </div> 
            ...
            <a class="btn btn-primary btn-sm ms-3 d-none d-md-block"
    ...
    <div class="offcanvas offcanvas-end w-25" tabindex="-1" id="offcanvasAddClass" aria-labelledby="offcanvasAddClassLabel">
        <div class="offcanvas-header">
            <div class="mainTitleWrapper d-flex align-items-center gap-3">
                ...
                <h5 class="offcanvas-title" id="offcanvasAddClassLabel">Add Books</h5>
            </div>
        ...
        <div class="offcanvas-body">
            <div class="box-body">
                ...
                <div class='form-group col-lg-12 mb-3' id='book_nameDiv'>
                    <label for="total_students" class="control-label">
                        Book Name <span class="text-danger">*</span>
                    </label>
                    <div class="">
                        <input type="text" class="form-control" id="book_name" name="book_name" onkeypress="return isText(event)">
                        <span class="control-label text-danger" id="book_name_error">
                    </div>
                </div>

                <div class='form-group col-lg-12 mb-3' id='authorIDDiv'>
                    <label for="total_students" class="control-label">
                        Author Name <span class="text-danger">*</span>
                    </label>
                    <div class="">
                        <?php
                            $authorArray = [ 0 => 'Select Author'];
                            
                            if(!empty($authors))
                            {
                                foreach ($authors as $author) 
                                {
                                    $authorArray[$author['authorID']] = $author['names'];
                                }
                            }
                            
                            echo form_dropdown("authorID", $authorArray, "authorID", "id='authorID' class='form-control select2'");
                        ?>
                        <span class="control-label text-danger" id="authorID_error"></span>
                    </div>
                </div>
    ...
    <!-- Add Book Offcanvas End -->

    <!-- Edit Book Offcanvas Start -->
    <div class="offcanvas offcanvas-end w-25" tabindex="-1" id="offcanvasEditClass" aria-labelledby="offcanvasEditClassLabel">
        <div class="offcanvas-header">
            <div class="mainTitleWrapper d-flex align-items-center gap-3">
                ...
                <h5 class="offcanvas-title" id="offcanvasEditClassLabel">Edit Books</h5>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div class="box-body">
                <!-- Your form inputs go here -->
                <div class='form-group col-lg-12 mb-3' id='edit_book_nameDiv'>
                    <label for="total_students" class="control-label">
                        Book Name <span class="text-danger">*</span>
                    </label>
                    <div class="">
                        <input type="text" class="form-control" id="edit_book_name" name="book_name" onkeypress="return isText(event)">
                        <span class="control-label text-danger" id="edit_book_name_error">
                    </div>
                </div>

                <div class='form-group col-lg-12 mb-3' id='edit_authorIDDiv'>
                    <label for="total_students" class="control-label">
                        Author Name <span class="text-danger">*</span>
                    </label>
                    <div class="">
                        <?php
                            ...
                            echo form_dropdown("edit_authorID", $authorArray, "edit_authorID", "id='edit_authorID' class='form-control select2'");
                        ?>
                        <span class="control-label text-danger" id="edit_authorID_error">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function get_books(page_no = 0, type = '')
    {
        var length    = $("#lengthSelect").val();
        var page_no   = page_no;
        var column    = ["#", "Book Name", "Author Name", "Create Date", "Action"];  
                ...
                if(result.status == true)
                {
                    ...
                    books.forEach(book => 
                    {
                        data += `<tr>
                                    ...
                                    <td>${book['date']}</td>
                                    <td>
                                        <button class="btn btn-sm btn-outline-secondary" onclick=edit('${btoa(JSON.stringify(book))}');>Edit</button>
                                        <button class="btn btn-sm btn-outline-danger" onclick=delete_modal(${book['bookID']})>Delete</button>
                                        <button class="btn btn-sm btn-outline-success" onclick=view_show('${btoa(JSON.stringify(book))}')>View</button>
                                    </td>
                                </tr>`
                    });
                    
                    html = datatable(data, result, page_no, column, "get_books");
    ...
    function edit(data)
    {
        ...
        $('#edit_authorID').val(authorID).trigger('change');
        $("#EditBtn").val(id);
    }

    function edit_name()
    {
        var name        = $("#edit_book_name").val().trim();
        var authorID    = $("#edit_authorID").val();
        var id          = $("#EditBtn").val();
        var errorCount  = 0

        if(name == '')
        {
            $("#edit_book_nameDiv").addClass('has-error');
            $("#edit_book_name_error").text("This filed is required !");
            errorCount++;
        }
        else
        {
            $("#edit_book_nameDiv").removeClass('has-error');
            $("#edit_book_name_error").text("");
        }

        if(authorID == 0)
        {
            $("#edit_authorIDDiv").addClass('has-error');
            $("#edit_authorID_error").text("This filed is required !");
            errorCount++;
        }
        else
        {
            $("#edit_authorIDDiv").removeClass('has-error');
            $("#edit_authorID_error").text("");
        }
    ...
    {
        var data    = JSON.parse(atob(data));
        $("#viewAuthorID").text(data.bookID);
        $("#viewAuthorName").text(data.names);
        $("#bknames").text(data.book_name);
